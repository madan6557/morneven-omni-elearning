...
export default function CourseDetail(){
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link to="/courses" className="text-sm text-zinc-500">← Kembali</Link>
      ...
      <div className="space-y-6">
        {c.modules?.map((m:any)=>(
          <div key={m.id} className="bg-white border rounded-2xl overflow-hidden">
            <div className="p-4 border-b bg-zinc-50 flex items-center justify-between">
              <div className="font-semibold">{m.title}</div>
              <div className="text-xs text-zinc-500">{m.materials.length} materi • {m.quizzes.length} quiz</div>
            </div>
            <div className="divide-y">
              {/* quizzes pre */}
              {m.quizzes?.filter((q:any)=>q.kind==="PRETEST").map((q:any)=>(
                <Link key={q.id} to={`/quiz/${q.id}`} className="flex items-center justify-between p-4 hover:bg-zinc-50">
                  <div className="flex gap-3 items-center">
                    <span className="w-9 h-9 rounded-full bg-amber-100 flex items-center justify-center">📝</span>
                    <div><div className="font-medium text-sm">{q.title}</div><div className="text-xs text-zinc-500">Pretest • {q.questions.length} soal • Lulus {q.passingScore}%</div></div>
                  </div>
                  <div className={`text-xs px-2 py-1 rounded-full ${quizDone.has(q.id)?"bg-green-100 text-green-700":"bg-zinc-100"}`}>{quizDone.has(q.id)?"Selesai":"Kerjakan"}</div>
                </Link>
              ))}
                ...
                return (
                  <Link key={mat.id} to={`/material/${mat.id}`} className="flex items-center gap-3 p-4 hover:bg-zinc-50">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-zinc-500 truncate">{mat.sourceType} • {mat.type} {mat.duration?`• ${Math.floor(mat.duration/60)}:${String(mat.duration%60).padStart(2,"0")}`:""} {mat.totalPages?`• ${mat.totalPages} hal`:""}</div>
                      <div className="mt-2 progress max-w-xs"><div style={{width:`${pct}%`}}/></div>
                    </div>
                    <div className="text-right">
                      <div className="text-xs font-medium">{Math.round(pct)}%</div>
                      {dlSet.has(mat.id) && <div className="text-[11px] text-green-600">downloaded</div>}
                    </div>
                  </Link>
                )
              })}
              ...
              {m.quizzes?.filter((q:any)=>q.kind==="POSTTEST"||q.kind==="QUIZ").map((q:any)=>(
                <Link key={q.id} to={`/quiz/${q.id}`} className="flex items-center justify-between p-4 hover:bg-zinc-50">
                  <div className="flex gap-3 items-center">
                    <span className="w-9 h-9 rounded-full bg-emerald-100 flex items-center justify-center">✅</span>
                    <div><div className="font-medium text-sm">{q.title}</div><div className="text-xs text-zinc-500">{q.kind} • {q.questions.length} soal</div></div>
                  </div>
                  <div className={`text-xs px-2 py-1 rounded-full ${quizDone.has(q.id)?"bg-green-100 text-green-700":"bg-zinc-100"}`}>{quizDone.has(q.id)?"Selesai":"Kerjakan"}</div>
                </Link>
              ))}

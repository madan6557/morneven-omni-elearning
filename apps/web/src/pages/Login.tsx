import { ArrowRight, BookOpen, CircleCheck, ChevronDown, ShieldCheck, Sparkles } from "lucide-react";
    ...
    <div className="app-shell grid min-h-screen lg:grid-cols-[1.05fr_0.95fr]">
      ...
      <section className="flex items-center justify-center px-5 py-10 sm:px-8">
        <form onSubmit={submit} className="surface w-full max-w-md rounded-3xl p-6 sm:p-8">
          ...
          <div className="space-y-5"><div><label htmlFor="nim" className="text-xs font-bold uppercase tracking-[0.12em] text-zinc-500 dark:text-zinc-400">NIM / Username</label><input id="nim" value={nim} onChange={(e) => setNim(e.target.value)} className="field mt-2" placeholder="Masukkan NIM atau username" autoComplete="username" /></div><div><label htmlFor="password" className="text-xs font-bold uppercase tracking-[0.12em] text-zinc-500 dark:text-zinc-400">Password</label><input id="password" type="password" value={pw} onChange={(e) => setPw(e.target.value)} className="field mt-2" placeholder="Masukkan password" autoComplete="current-password" /></div></div>

      ...
      {course && (
        <>
          ...
          <section className="section-card space-y-5">
            ...
            <button disabled={pending !== null} onClick={async () => { setPending("material"); try { await api.post("/api/materials", { moduleId: mat.moduleId, title: mat.title, type: mat.type, sourceType: mat.sourceType, sourceUrl: mat.sourceUrl, duration: mat.duration ? Number(mat.duration) : undefined, totalPages: mat.totalPages ? Number(mat.totalPages) : undefined }); setMat((m) => ({ ...m, title: "", sourceUrl: "", duration: "", totalPages: "" })); await loadCourse(course.id); showFeedback("Materi berhasil disimpan."); } catch { showFeedback("Materi gagal disimpan.", "error"); } finally { setPending(null); } }} className="primary-button disabled:cursor-wait disabled:opacity-70">{pending === "material" ? <Spinner /> : <Save size={17} />} {pending === "material" ? "Menyimpan..." : "Simpan materi"}</button>

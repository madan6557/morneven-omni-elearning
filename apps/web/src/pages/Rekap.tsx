...
export default function Rekap() {
  ...
  const exportCsv = () => {
    ...
    const rows = data.rekap.map((r: any) => { const submittedAttempts = r.attempts.filter((item: any) => item.submittedAt); const gradedSubmissions = (r.submissions || []).filter((item: any) => item.score !== null); return [r.user.nim, r.user.name, `${r.overall}%`, r.downloads.length, r.videos.length, r.videos.length ? `${Math.round(r.videos.reduce((sum: number, item: any) => sum + item.percent, 0) / r.videos.length)}%` : "0%", r.slides.length, r.attempts.length, new Set(submittedAttempts.map((item: any) => item.quizId)).size, submittedAttempts.length ? `${Math.max(...submittedAttempts.map((item: any) => item.score))}%` : "-", r.submissions?.length || 0, gradedSubmissions.length, gradedSubmissions.length ? `${Math.round(gradedSubmissions.reduce((sum: number, item: any) => sum + item.score, 0) / gradedSubmissions.length)}%` : "-"].map(quote).join(","); }).join("\n");

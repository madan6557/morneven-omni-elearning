  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        <select value={sel} onChange={e=>setSel(e.target.value)} className="border rounded-lg px-3 py-2 bg-white">
        ...
        }} className="px-4 py-2 rounded-lg bg-zinc-900 text-white text-sm">Export CSV</button>}
      </div>
      {data && (
        <div className="bg-white border rounded-2xl overflow-auto">
          <table className="w-full text-sm">
            <thead className="bg-zinc-50"><tr><th className="p-3 text-left">NIM</th><th className="p-3 text-left">Nama</th><th className="p-3">Overall</th><th className="p-3">Download</th><th className="p-3">Video</th><th className="p-3">Slide</th><th className="p-3">Quiz</th></tr></thead>
            <tbody>
              {data.rekap.map((r:any)=>(
                <tr key={r.user.id} className="border-t">
                  ...
                  <td className="p-3 text-center">{r.videos.length} <span className="text-xs text-zinc-500">({r.videos.map((v:any)=>Math.round(v.percent)+"%").join(", ")})</span></td>

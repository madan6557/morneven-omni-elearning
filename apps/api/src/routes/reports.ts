...
async function buildReport(req: any) {
  ...
  const enrollments = await prisma.enrollment.findMany({ where: { courseId }, include: { user: { select: { id: true, nim: true, name: true, role: true } } }, orderBy: { enrolledAt: "asc" } });
  ...
  for (const enrollment of enrollments) {
    ...
    const materialRows = materials.map((item) => ({ nim: enrollment.user.nim, name: enrollment.user.name, module: modules.find((mod) => mod.id === item.moduleId)?.title || "", material: item.title, type: item.type, progress: Math.round(Number(item.type === "VIDEO" ? videoMap.get(item.id) || 0 : slideMap.get(item.id) || 0)), downloaded: downloads.some((download) => download.materialId === item.id) }));
    ...
    attemptsOut.push(...attempts.map((item) => ({ nim: enrollment.user.nim, name: enrollment.user.name, quiz: item.quiz.title, kind: item.quiz.module?.type || item.quiz.kind, module: item.quiz.module?.title || "", attemptId: item.id, startedAt: item.startedAt.toISOString(), submittedAt: item.submittedAt?.toISOString() || "", score: item.score, passed: item.passed, essayStatus: item.answerGrades.length ? `${item.answerGrades.length} essay dinilai` : "Tidak ada penilaian essay" })));
    submissionsOut.push(...submissions.map((item) => ({ nim: enrollment.user.nim, name: enrollment.user.name, module: item.assignment.module?.title || "", assignment: item.assignment.title, deadline: item.assignment.deadline?.toISOString() || "", submittedAt: item.submittedAt.toISOString(), score: item.score ?? "", gradingStatus: item.score === null ? "Belum dinilai" : "Sudah dinilai", feedback: item.feedback || "", file: item.fileName || item.fileUrl || "", link: item.externalUrl || "" })));
